 ...
 */
export const DustbinDropZone = ({ active, point, onHover }) => {
    ...
    }, [active, point, onHover]);

    return (
        <div
            aria-hidden={!active}
            className={`fixed left-3 sm:left-6 top-1/2 -translate-y-1/2 z-[55] pointer-events-none transition-all duration-300 ${active ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-12'
                }`}
        >
            <div
                ref={ref}
                id="receipt-dustbin-target"
                className={`relative w-20 h-24 sm:w-24 sm:h-28 rounded-2xl flex items-end justify-center px-2 pb-3 transition-all duration-200 ${over
            ...
            >
                {/* Bin lid */}
                <div
                    className={`absolute -top-2 left-1/2 -translate-x-1/2 w-[110%] h-2.5 rounded-t-md transition-transform duration-200 origin-bottom-left ${over ? 'bg-red-400 -rotate-12 -translate-x-[60%] -translate-y-1' : 'bg-purple-400'
                        }`}
                />
                {/* Lid handle */}
                <div
                    className={`absolute -top-3 left-1/2 -translate-x-1/2 w-4 h-1.5 rounded-full transition-transform duration-200 ${over ? 'bg-red-300 -rotate-12 -translate-x-[60%] -translate-y-1' : 'bg-purple-300'
                        }`}
                />

                {/* Trash icon */}
                <Trash2
                    className={`w-9 h-9 sm:w-10 sm:h-10 mb-1 transition-colors duration-200 ${over ? 'text-white' : 'text-purple-100'
                        }`}
                />
            ...
            <p
                className={`mt-2 text-center text-[11px] font-semibold tracking-wide transition-colors ${over ? 'text-red-300' : 'text-purple-200'
                    }`}
            >
                {over ? 'Release to delete' : 'Drop here'}
            </p>
        </div>
    );
};
